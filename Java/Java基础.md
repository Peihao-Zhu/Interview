...
在说Class类之前需要了解.class文件是由类加载器加载进来的。然后JVM通过其字节数组创建了Class对象。
...
  Class对象包括了类的所有信息，如下图所示是经过编译以后的.class文件，只是变成了字节数组。

![image-20210407132842734](https://raw.githubusercontent.com/Peihao-Zhu/blogImage/master/data/image-20210407132842734.png)

然后看一下对应的Class对象。

![img](https://pic4.zhimg.com/80/v2-fe965a307866fddee50862bbe8b3eabb_1440w.jpg)

因为字段、方法、构造器的信息很多，所以java.lang.reflect包下包含了三个类描述这些信息
...
![img](https://pic3.zhimg.com/80/v2-c7cc3cdbc4d43ad4d81196a6da5ee8b6_1440w.jpg)

这边第一个参数方法名可以理解，那么第二个参数类型为什么要用Class类型呢？

因为可能一个类里面存在重载，同名方法很多，需要根据相关的参数类型来区分。

那么能不能用String，int这种类型来传呢，不用Class对象？

不好意思，这些都是基本类型和引用类型，类型不能用来传递。我们能传递的要么值，要么对象（引用）。而String.class, int.class是对象，且是Class对象。

实际上调用Class对象的getMethod()方法时，内部会循环遍历所有的Method，找到匹配的Method对象返回。

- **调用method.invoke(obj, args);时为什么要传入一个目标对象？**

这是在获取了Method对象以后，要执行对应的方法时，传入的obj。因为方法时共用的，所以Method()能被实例对象调用，要保证p1对象使用方法的时候对p2对象不会进行更改。

所以JVM设置了一种隐性机制，每次对象调用方法时，都会隐性传递当前调用该方法的对象参数(this)，方法可以根据这个对象参数知道当前调用本方法的是哪个对象！

所以在这里需要传入执行这个方法的对象。

> 如果invoke一个静态方法，不需要传入具体的对象。因为静态方法不能处理实例对象中保存的数据。
